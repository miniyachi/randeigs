...
% Define saving stems
path_experiment_matclass = fullfile('m_varies', matclass);
...
clear_summary = true;
summary_dir = fullfile(fig_stem, 'Summary', path_metric, path_maxiter);
...
for i = 1:length(name_list)
    ...
    max_end = 4000;
    size_factor = 100 * floor(size(A,1) / 100 - 1);
    endpoint = min(max_end, size_factor);
    
    % Generate the list
    m_list = [1000:1000:endpoint]';
    ...
    if saved
        ...
        save(fullfile(results_name_stem, filename_errors), 'classical_errs', 'rand_errs', 'maxiter', ...
